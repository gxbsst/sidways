...
def send_email params, ip_address
  ...
  body = Liquid::Template.parse(email_template).render  "name"       => params[:name], 
                                                        ...
                                                        Pony.mail(:to => "info@sidways.com", :from => params[:email], :subject => "A comment from #{params[:name]}", :body => body)
end


DataMapper::setup(:default, "sqlite3://#{Dir.pwd}/sidways.db")
...
post '/create' do
  #params[:feedback]["ip"] =  ENV['REMOTE_ADDR']
  #params[:feedback]["user_agent"] =  ENV['HTTP_USER_AGENT']

  #@feedback = Feedback.new(params[:feedback])
  #@feedback.ip = @env['REMOTE_ADDR']
  #@feedback.user_agent =  @env['HTTP_USER_AGENT']

  #if @feedback.save
    begin
      send_email(params[:feedback], @env['REMOTE_ADDR']) 
      @sent = true
